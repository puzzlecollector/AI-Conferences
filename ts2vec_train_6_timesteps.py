...
lookback = 6 # worth 4 * 6 = 24 hours 
X_train_seq = [] 
for i in range(lookback, train_size): 
    X_train_seq.append(X_train.values[i-lookback:i]) 

X_train_seq = np.array(X_train_seq).reshape((-1, lookback, num_features)) 
print(X_train_seq.shape, X_train_seq.ndim) 

model = TS2Vec(input_dims=num_features, device=0, output_dims=128) 

loss_log = model.fit(X_train_seq, n_epochs=200, verbose=True) 
model.save("ts2vec_embedder_XGB.pt") 

print("done training and saved checkpoint!") 