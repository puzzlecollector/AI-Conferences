...
model = TFN(n_var_past_cont = 11, 
            n_var_future_cont = 10, 
...
# load data 
past_cont_inputs = np.load("aug_top10_simcse_past_cont_inputs.npy")
past_cont_inputs = torch.tensor(past_cont_inputs).float() 

past_dates = np.load("aug_top10_simcse_past_dates.npy")
past_dates = torch.tensor(past_dates).float() 

future_cont_inputs = np.load("aug_top10_simcse_future_cont_inputs.npy") 
future_cont_inputs = torch.tensor(future_cont_inputs).float() 

future_dates = np.load("aug_top10_simcse_future_dates.npy") 
future_dates = torch.tensor(future_dates).float() 

targets = np.load("aug_top10_simcse_targets.npy") 
targets = torch.tensor(targets).float() 

# convert to ratios 
past_cont_inputs_r = [] 
for i in tqdm(range(1, past_cont_inputs.shape[0]), position=0, leave=True, desc="Calculating"): 
    ratios = torch.zeros((1, 24, 11))
    for j in range(0, 24):
        if j == 0: 
            for k in range(11): 
                ratios[:, j, k] = past_cont_inputs[i, j, k] / past_cont_inputs[i-1, 0, k] 
        elif j > 0: 
            for k in range(11): 
                ratios[:, j, k] = past_cont_inputs[i, j, k] / past_cont_inputs[i, j-1, k]  
    past_cont_inputs_r.append(ratios)
past_cont_inputs_r = torch.cat(past_cont_inputs_r, dim=0).float()   

future_cont_inputs_r = [] 
for i in tqdm(range(future_cont_inputs.shape[0]), position=0, leave=True, desc="Calculating"): 
    ratios = torch.zeros((1, 6, 10)) 
    for j in range(0, 6): 
        for k in range(10): 
            if j == 0: 
                ratios[:, j, k] = future_cont_inputs[i, j, k] / past_cont_inputs[i, -1, k+1] 
            elif j > 0: 
                ratios[:, j, k] = future_cont_inputs[i, j, k] / future_cont_inputs[i, j-1, k]   
    future_cont_inputs_r.append(ratios) 
future_cont_inputs_r = torch.cat(future_cont_inputs_r, dim=0).float() 

targets_r = [] 
for i in tqdm(range(targets.shape[0]), position=0, leave=True, desc="Calculating"):
    ratios = torch.zeros((1, 6)) 
    for j in range(0, 6): 
        if j == 0: 
            ratios[:, j] = targets[i, j] / past_cont_inputs[i, -1, 0] 
        elif j > 0: 
            ratios[:, j] = targets[i, j] / targets[i, j-1] 
    targets_r.append(ratios) 
targets_r = torch.cat(targets_r, dim=0).float() 
...
train_size = int(0.8 * past_cont_inputs_r.shape[0]) 
val_size = int(0.1 * past_cont_inputs_r.shape[0]) 

train_past_cont_inputs = past_cont_inputs_r[:train_size]
train_past_dates = past_dates[:train_size] 
train_future_cont_inputs = future_cont_inputs_r[:train_size] 
train_future_dates = future_dates[:train_size] 
train_targets = targets_r[:train_size] 

val_past_cont_inputs = past_cont_inputs_r[train_size:train_size+val_size] 
val_past_dates = past_dates[train_size:train_size+val_size] 
val_future_cont_inputs = future_cont_inputs_r[train_size:train_size+val_size] 
val_future_dates = future_dates[train_size:train_size+val_size] 
val_targets = targets_r[train_size:train_size+val_size] 

test_past_cont_inputs = past_cont_inputs_r[train_size+val_size:] 
test_past_dates = past_dates[train_size+val_size:] 
test_future_cont_inputs = future_cont_inputs_r[train_size+val_size:] 
test_future_dates = future_dates[train_size+val_size:] 
test_targets = targets_r[train_size+val_size:] 

print(train_past_cont_inputs.shape, val_past_cont_inputs.shape, test_past_cont_inputs.shape, train_past_dates.shape, val_past_dates.shape, test_past_dates.shape)  

    
train_dataset = CustomDataset(train_past_cont_inputs, train_past_dates, train_future_cont_inputs, train_future_dates, train_targets) 
train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True) 

val_dataset = CustomDataset(val_past_cont_inputs, val_past_dates, val_future_cont_inputs, val_future_dates, val_targets) 
val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size, shuffle=False) 

test_dataset = CustomDataset(test_past_cont_inputs, test_past_dates, test_future_cont_inputs, test_future_dates, test_targets) 
...
try: 
    ...
    for epcoh_i in tqdm(range(0, epochs), desc="Epochs", position=0, leave=True, total=epochs):
        ...
        val_loss = 0 
        model.eval() 
        for step, batch in tqdm(enumerate(val_dataloader), position=0, leave=True, total=len(val_dataloader), desc="Validating"): 
            past_cont = batch["past_cont"].to(device) 
            past_disc = batch["past_disc"].to(device) 
            future_cont = batch["future_cont"].to(device) 
            future_disc = batch["future_disc"].to(device) 
        ...
        if avg_val_loss < best_val_loss: 
            ...
            torch.save(model.state_dict(), "32TFT_SIMCSE_ranked.pt") 
except KeyboardInterrupt:
    print(f"current best validation loss : {best_val_loss}")
    
os.rename("32TFT_SIMCSE_ranked.pt", f"32TFT_SIMCSE_ranked_{best_val_loss}.pt")

print("saving arrays") 
with open('SIMCSE_train_losses.pkl', 'wb') as f:
    pickle.dump(train_losses, f)
    
with open('SIMCSE_val_losses.pkl', "wb") as f:
