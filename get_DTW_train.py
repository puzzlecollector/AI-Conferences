...
def DTW_distance(x1, x2): 
    ...
    return dtw.dtw(rel_x1, rel_x2, keep_internals=True).distance 
    #distance, path = fastdtw(rel_x1, rel_x2, dist=euclidean)
    #return distance 
...
for i in tqdm(range(start_index, chart_df.shape[0]), position=0, leave=True, desc="Total number of samples"):
    ...
    for j in range(0, i): 
        ...
        dist = DTW_distance(query_ts, candidate_ts) 
...
with open("xgb_chart_similar_dates_DTW.pkl", "wb") as f: 
