...
def euclidean_distance(x1, x2): 
    rel_x1 = x1[comp_columns].values 
    rel_x2 = x2[comp_columns].values  
    return np.linalg.norm(rel_x1 - rel_x2) 
...
for i in tqdm(range(start_index, chart_df.shape[0]), position=0, leave=True):
    query_date = all_dates[i] 
    query_ts = chart_df.iloc[i] 
    candidate_info = [] 
    for j in range(0, i): 
        candidate_ts = chart_df.iloc[j] 
        candidate_date = all_dates[j] 
        dist = euclidean_distance(query_ts, candidate_ts) 
...
with open("xgb_chart_similar_dates.pkl", "wb") as f: 
