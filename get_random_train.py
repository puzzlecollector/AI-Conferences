...
for idx in tqdm(range(iters), position=0, leave=True): 
    ...
    for i in range(start_index, chart_df.shape[0]):
        query_date = all_dates[i] 
        query_ts = chart_df.iloc[i] 

        candidate_info = [] 

        randomly_chosen = random.sample(range(i), max_topk)
        for j in range(len(randomly_chosen)): 
            random_idx = randomly_chosen[j] 
            candidate_date = all_dates[random_idx] 
            candidate_info.append(candidate_date) 

        similar_dates[query_date] = candidate_info 

    with open(f"random_dates/random_similar_dates_{idx+1}.pkl", "wb") as f: 
