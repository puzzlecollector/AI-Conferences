...
# load data 
past_cont_inputs = np.load("random_top3_past_cont_inputs.npy")
past_cont_inputs = torch.tensor(past_cont_inputs).float() 

past_dates = np.load("random_top3_past_dates.npy")
past_dates = torch.tensor(past_dates).float() 

future_cont_inputs = np.load("random_top3_future_cont_inputs.npy") 
future_cont_inputs = torch.tensor(future_cont_inputs).float() 

future_dates = np.load("random_top3_future_dates.npy") 
...
train_size = int(0.8 * past_cont_inputs.shape[0]) 
val_size = int(0.1 * past_cont_inputs.shape[0]) 

train_past_cont_inputs = past_cont_inputs[:train_size]
train_past_dates = past_dates[:train_size] 
train_future_cont_inputs = future_cont_inputs[:train_size] 
train_future_dates = future_dates[:train_size] 
train_targets = targets[:train_size] 

val_past_cont_inputs = past_cont_inputs[train_size:train_size+val_size] 
val_past_dates = past_dates[train_size:train_size+val_size] 
val_future_cont_inputs = future_cont_inputs[train_size:train_size+val_size] 
val_future_dates = future_dates[train_size:train_size+val_size] 
val_targets = targets[train_size:train_size+val_size] 

test_past_cont_inputs = past_cont_inputs[train_size+val_size:] 
test_past_dates = past_dates[train_size+val_size:] 
test_future_cont_inputs = future_cont_inputs[train_size+val_size:] 
...
try: 
    ...
    for epcoh_i in tqdm(range(0, epochs), desc="Epochs", position=0, leave=True, total=epochs):
        ...
        if avg_val_loss < best_val_loss: 
            ...
            torch.save(model.state_dict(), "TFT_RANDOM_ranked.pt") 
except KeyboardInterrupt:
    print(f"current best validation loss : {best_val_loss}")
    
os.rename("TFT_RANDOM_ranked.pt", f"TFT_RANDOM_ranked_{best_val_loss}.pt")

print("saving arrays") 
with open('train_losses_RANDOM.pkl', 'wb') as f:
    pickle.dump(train_losses, f)
    
with open('val_losses_RANDOM.pkl', "wb") as f:
