...
for col in chart_df.columns:
    if col not in ["Targets", "datetime", "years", "hours", "days", "months"]:
...
# we only train using the train dataset 
num_features = X_train.shape[1] 

num_features = X_train.shape[1] 
model = TS2Vec(input_dims=num_features, output_dims=128) 
model.load("ts2vec_embedder_XGB.pt")
...
for i in tqdm(range(lookback, len(all_dates)), position=0, leave=True):
    output = model.encode(X.values[i-lookback:i].reshape((-1, lookback, num_features))) 
    output = np.mean(output, axis=1) 
    date_chart_emb[all_dates[i]] = output 
    
with open("all_ts2vec_embeddings.pkl", "wb") as f: 
    pickle.dump(date_chart_emb, f) 
...
for i in tqdm(range(start_index, chart_df.shape[0]), position=0, leave=True, desc="Total number of samples"):
    ...
    query_emb = date_chart_emb[query_date] 
    candidate_info = [] 
    for j in range(lookback, i): 
        candidate_date = all_dates[j] 
        candidate_emb = date_chart_emb[candidate_date] 
...
with open("ts2vec_similar_dates.pkl", "wb") as f: 
