...
for i in range(0, chart_df.shape[0] - seq_len):
        X_seq.append(chart_df.iloc[i:i+seq_len, 3].values) # get close values only 
...
def custom_collate(batch):
    ...
    ids = 0 
    for seq in batch: 
        sequences.append(torch.reshape(seq[0], (-1, seq_len, 1))) 
        sequences.append(torch.reshape(seq[0], (-1, seq_len, 1))) 
        labels.append(ids) 
        labels.append(ids) 
        ids += 1 
...
for epoch_i in tqdm(range(epochs), desc="Epochs", position=0, leave=True, total=epochs):
    ...
    if avg_val_loss < best_val_loss: 
        ...
        torch.save(model.state_dict(), "BTC_SIMCSE_1000.pt") 

print("done training!") 
print(f"best val loss : {best_val_loss}") 
os.rename("BTC_SIMCSE_1000.pt", f"BTC_SIMCSE_1000_val_loss_{best_val_loss}.pt") 
