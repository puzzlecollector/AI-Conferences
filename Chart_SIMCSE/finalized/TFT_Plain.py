...
model = TFN(n_var_past_cont = 1, 
            ...
            num_features=1, 
            forecast_horizon=6,
            future_num_features=0) 

model.to(device) 

# load data 
past_cont_inputs = np.load("non_augmented_past_cont_inputs.npy")
past_cont_inputs = torch.tensor(past_cont_inputs).float() 

past_dates = np.load("non_augmented_past_dates.npy")
past_dates = torch.tensor(past_dates).float() 

future_dates = np.load("non_augmented_future_dates.npy") 
future_dates = torch.tensor(future_dates).float() 

targets = np.load("non_augmented_targets.npy") 
...
train_dataset = CustomDataset(train_past_cont_inputs, train_past_dates, train_future_dates, train_targets) 
train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True) 

val_dataset = CustomDataset(val_past_cont_inputs, val_past_dates, val_future_dates, val_targets) 
val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size, shuffle=False) 

test_dataset = CustomDataset(test_past_cont_inputs, test_past_dates, test_future_dates, test_targets) 
...
try: 
    ...
    for epcoh_i in tqdm(range(0, epochs), desc="Epochs", position=0, leave=True, total=epochs):
        ...
        if avg_val_loss < best_val_loss: 
            ...
            torch.save(model.state_dict(), "TFT_no_augmentation.pt") 
except KeyboardInterrupt:
    print(f"current best validation loss : {best_val_loss}")
    

os.rename("TFT_no_augmentation.pt", f"TFT_no_augmentation_{best_val_loss}.pt")

print("saving arrays") 
with open('train_losses_augmentation.pkl', 'wb') as f:
    pickle.dump(train_losses, f)
    
with open('val_losses_augmentation.pkl', "wb") as f:
